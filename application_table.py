...
class Table(Frame):  # Изменено: Table теперь наследуется от Frame
    ...
    def create_table(self):
        ...
        last_col=1
        #список дат
        for task_name,task_value in self.employee_list[list(self.employee_list.keys())[1]].items():
            if last == None:
                last = task_value["task date"]
                last_col = list(self.employee_list[list(self.employee_list.keys())[1]].keys()).index(task_name)+1
            elif last != task_value["task date"] and len(self.employee_list[list(self.employee_list.keys())[1]].keys()) -1 > list(self.employee_list[list(self.employee_list.keys())[1]].keys()).index(task_name):
                col_span = list(self.employee_list[list(self.employee_list.keys())[1]].keys()).index(task_name) - last_col + 1
                self.entry=Entry(
                        self,
                        justify='center',
                ...
                self.entry.grid(row=0, column=last_col,columnspan=col_span, sticky='nsew')
                last = task_value["task date"]
                last_col = list(self.employee_list[list(self.employee_list.keys())[1]].keys()).index(task_name)+1
            elif last != task_value["task date"] and len(list(self.employee_list[list(self.employee_list.keys())[1]].keys())) -1 == list(self.employee_list[list(self.employee_list.keys())[1]].keys()).index(task_name):
                col_span = list(self.employee_list[list(self.employee_list.keys())[1]].keys()).index(task_name) - last_col + 1
                self.entry=Entry(
                        self,
                        justify='center',
                ...
                #print(total_columns,total_rows,count_cell,count_cell//total_rows, count_cell%total_columns)
                self.entry.grid(row=0, column=last_col+col_span, sticky='nsew')
                last = task_value["task date"]
                last_col = list(self.employee_list[list(self.employee_list.keys())[1]].keys()).index(task_name)+1
            elif last==task_value["task date"] and len(list(self.employee_list[list(self.employee_list.keys())[1]].keys())) -1 == list(self.employee_list[list(self.employee_list.keys())[1]].keys()).index(task_name):
                col_span = list(self.employee_list[list(self.employee_list.keys())[1]].keys()).index(task_name) - last_col + 2
                self.entry=Entry(
                            self,
                            justify='center',
                            width=cell_width*col_span,
        ...
        for task_name in self.employee_list["task"]:
            if last == None:
            ...
            elif last != task_name.split("-")[0] and len(self.employee_list["task"]) -1 > self.employee_list["task"].index(task_name):
                col_span = self.employee_list["task"].index(task_name) - last_col + 1
                self.entry=Entry(
                        self,
                        justify='center',
                        width=cell_width*col_span,
                ...
                self.entry.insert(END,last)
                #print(total_columns,total_rows,count_cell,count_cell//total_rows, count_cell%total_columns)
                self.entry.grid(row=1, column=last_col,columnspan=col_span, sticky='nsew')
                last = task_name.split("-")[0]
                last_col = self.employee_list["task"].index(task_name)+1
            elif last != task_name.split("-")[0] and len(self.employee_list["task"]) -1 == self.employee_list["task"].index(task_name):
                col_span = self.employee_list["task"].index(task_name) - last_col + 1
                ...
                #print(total_columns,total_rows,count_cell,count_cell//total_rows, count_cell%total_columns)
                self.entry.grid(row=1, column=last_col+col_span, sticky='nsew')
                last = task_name.split("-")[0]
                last_col = self.employee_list["task"].index(task_name)+1
            elif last==task_name.split("-")[0] and len(self.employee_list["task"]) -1 == self.employee_list["task"].index(task_name):
                col_span = self.employee_list["task"].index(task_name) - last_col + 2
                self.entry=Entry(
                            self,
                            justify='center',
                            width=cell_width*col_span,
        ...
        for student_name,task_name in self.employee_list.items():
            if student_name=="task":
                ...
                self.entry=Entry(
                        self,
                        width=cell_width if count_cell%(self.total_columns-self.add_columns) != 0 else max_len_column_0,
                        bg=self.table_cell_color(count_cell//self.total_columns+self.row_add,count_cell%self.total_columns),
                        fg="Black",
                        font=("Arial", 16, "bold"),
                        borderwidth=self.table_borderwidth,
                        relief=self.table_relief_baze,
                ...
                self.entry.insert(END,self.table_cell_text(count_cell//self.total_columns+self.row_add,count_cell%self.total_columns))
                #print(total_columns,total_rows,count_cell,count_cell//total_rows, count_cell%total_columns)
                self.entry.grid(row=count_cell//(self.total_columns-self.add_columns), column=count_cell%(self.total_columns-self.add_columns),rowspan=3, sticky='nsew')
                self.event_table(self.entry,count_cell//self.total_columns+self.row_add,count_cell%self.total_columns)
                
                count_cell+=1
                for i in task_name:
                    self.entry=Entry(
                        self,
                        width=cell_width if count_cell%self.total_columns != 0 else max_len_column_0,
                        bg=self.table_cell_color(count_cell//self.total_columns+self.row_add,count_cell%self.total_columns),
                        fg="Black",
                        font=("Arial", 16, "bold"),
                        justify='center',
                    ...
                    self.entry.grid(row=count_cell//self.total_columns+self.row_add, column=count_cell%self.total_columns, sticky='nsew')
                    self.event_table(self.entry,count_cell//self.total_columns+self.row_add,count_cell%self.total_columns)
                    count_cell+=1
                ...
                for i in range(self.total_columns-count_cell):
                    ...
                    #print(total_columns,total_rows,count_cell,count_cell//total_columns, count_cell%total_columns)
                    self.entry.grid(row=count_cell//(self.total_columns), column=count_cell%(self.total_columns),rowspan=3, sticky='nsew')
                    self.event_table(self.entry,count_cell//self.total_columns+self.row_add,count_cell%self.total_columns)
                    count_cell+=1
            else:
                ...
                self.event_table(self.entry,count_cell//self.total_columns+self.row_add,count_cell%self.total_columns)
                count_cell+=1
                for i in range(self.total_columns-(count_cell%self.total_columns)):
                    self.entry=Entry(
                        self,
                        width=cell_width if count_cell%self.total_columns != 0 else max_len_column_0,
                        bg=self.table_cell_color(count_cell//self.total_columns+self.row_add,count_cell%self.total_columns),
    ...
    def event_table(self,tmp,row,column):
        tmp.bind('<Enter>', lambda event, row=row, column=column: self.highlight_row_and_column(row, column))
        tmp.bind('<Leave>', self.clear_highlight)
        tmp.bind('<Button-1>',lambda event,row=row-self.row_add, column=column: self.open_code_student(row,column))
        tmp.bind('<Return>', lambda event, row=row,column=column: self.enter_grade(row,column))
    ...
    def enter_grade(self,row,column):
        ...
        list(self.employee_list.items())[row-self.row_add][1][self.employee_list["task"][column-1]]["grade"] = tmp
    ...
    def highlight_row_and_column(self, row, column):
        ...
        for i in range(self.row_add,self.total_rows):
            if self.table_cell_color(i,column) == self.table_cell_color_green:
            ...
            elif self.table_cell_color(i,column) == self.table_cell_color_yellow:
                ...
                self.grid_slaves(row=i)[-(column+1)].configure(bg=self.blend_colors(self.table_cell_color(i,column),self.table_highlight_color))  # Голубой цвет для ячеек
        ...
        for j in range(self.total_columns):
            ...
            if self.table_cell_color(row,j) == self.table_cell_color_green:
                ...
                self.grid_slaves(row=row)[-(j+1)].configure(bg=self.blend_colors(self.table_cell_color(row,j),self.table_highlight_color))  # Голубой цвет для ячеек
    ...
    def clear_highlight(self, event=None):
        if self.selected_row is not None and self.selected_column is not None:
            ...
            for i in range(self.row_add,self.total_rows):
                ...
                self.grid_slaves(row=i)[-(self.selected_column+1)].configure(bg=self.table_cell_color(i,self.selected_column))
            for j in range(self.total_columns):
                #self.grid_slaves(row=self.selected_row)[-(j+1)].configure(relief=self.table_relief_baze)
                self.grid_slaves(row=self.selected_row)[-(j+1)].configure(bg=self.table_cell_color(self.selected_row,j))
    ...
    def table_cell_color(self,row,column):
        ...
        if row < 1:
            ...
            if list(self.employee_list.items())[row][1][self.employee_list["task"][column-1]]["done"] == None:
                return self.table_cell_color_white
            elif list(self.employee_list.items())[row][1][self.employee_list["task"][column-1]]["done"] == False:
                return self.table_cell_color_white
            elif list(self.employee_list.items())[row][1][self.employee_list["task"][column-1]]["grade"] != None:
                if list(self.employee_list.items())[row][1][self.employee_list["task"][column-1]]["grade"] == 1.0:
                    return self.table_cell_color_green
                elif 0<list(self.employee_list.items())[row][1][self.employee_list["task"][column-1]]["grade"] <1:
                    return self.table_cell_color_yellow
                else:
                    return self.table_cell_color_red
            elif list(self.employee_list.items())[row][1][self.employee_list["task"][column-1]]["auto_check"] == True:
                return self.table_cell_color_green
            elif list(self.employee_list.items())[row][1][self.employee_list["task"][column-1]]["auto_check"] == False:
                return self.table_cell_color_red
            elif list(self.employee_list.items())[row][1][self.employee_list["task"][column-1]]["done"] == True and \
                list(self.employee_list.items())[row][1][self.employee_list["task"][column-1]]["auto_check"] == None:
                return self.table_cell_color_yellow
    ...
    def table_cell_text(self,row,column):
        ...
        if row == 0:
            if column==0:
                ...
                return self.add_columns_name[column-len(self.employee_list["task"])-1]
        elif row >0:
            if column==0:
                return list(self.employee_list.keys())[row]
            elif column < len(self.employee_list['task'])+1:
                if list(self.employee_list.items())[row][1][self.employee_list["task"][column-1]]["done"]==False or\
                    list(self.employee_list.items())[row][1][self.employee_list["task"][column-1]]["done"] == None:
                    return ''
                elif list(self.employee_list.items())[row][1][self.employee_list["task"][column-1]]["done"] == True:
                    if list(self.employee_list.items())[row][1][self.employee_list["task"][column-1]]["auto_check"] == False:
                        return 0
                    elif list(self.employee_list.items())[row][1][self.employee_list["task"][column-1]]["auto_check"] == None:
                        return '+'
                    elif list(self.employee_list.items())[row][1][self.employee_list["task"][column-1]]["auto_check"] == True:
                        if list(self.employee_list.items())[row][1][self.employee_list["task"][column-1]]["grade"] == None:
                            return ''
                        elif list(self.employee_list.items())[row][1][self.employee_list["task"][column-1]]["grade"] != None:
                            if list(self.employee_list.items())[row][1][self.employee_list["task"][column-1]]["grade"] == 1.0:
                                return 1
                            elif list(self.employee_list.items())[row][1][self.employee_list["task"][column-1]]["grade"] == 0.0:
                                return 0
                            else:
                                return list(self.employee_list.items())[row][1][self.employee_list["task"][column-1]]["grade"]
            elif self.add_columns_name[column-len(self.employee_list['task'])-1] == self.add_columns_name[0]:
                grade = 0
                for j in range(len(self.employee_list['task'])):
                    if list(self.employee_list.items())[row][1][self.employee_list["task"][j]]["grade"] != None:
                        grade+=list(self.employee_list.items())[row][1][self.employee_list["task"][j]]["grade"]
                if grade==0:
                    return ''
                else:
            ...
            elif self.add_columns_name[column-len(self.employee_list['task'])-1] == self.add_columns_name[2]:
                return self.table_pvr(row)
            elif self.add_columns_name[column-len(self.employee_list['task'])-1] == self.add_columns_name[3]:
    ...
    def table_teacher_check(self,row):
        ...
        for j in range(len(self.employee_list['task'])):
            if  list(self.employee_list.items())[row][1][self.employee_list["task"][j]]["teacher_check"] != True:
                return False
        else:
            return True
    ...
    def table_pv(self,row):
        ...
        for j in range(len(self.employee_list['task'])):
            ...
            if list(self.employee_list.items())[row][1][self.employee_list["task"][j]]["grade"] != None:
                grade+=list(self.employee_list.items())[row][1][self.employee_list["task"][j]]["grade"]
            if  list(self.employee_list.items())[row][1][self.employee_list["task"][j]]["auto_check"] == True:
                c+=1
    ...
    def table_pvr(self,row):
        ...
        grade = 0
        for j in range(len(self.employee_list['task'])):
            if list(self.employee_list.items())[row][1][self.employee_list["task"][j]]["done"] == True:
                ct+=1
            if list(self.employee_list.items())[row][1][self.employee_list["task"][j]]["grade"] != None:
                grade+=list(self.employee_list.items())[row][1][self.employee_list["task"][j]]["grade"]
        if ct==0:
