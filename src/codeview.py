...
class ViewCode(Frame):
    ...
    def create_widget(self):
        ...
        self.problem_text_label = Label(
            frame_top, text="Задача :", font=font_size_label
        )
        self.problem_text_label.grid(
            row=0, column=0, columnspan=2, padx=10, sticky="w"
        )

        self.problem_text = Text(
            frame_top, width=80, height=5, font=font_size_text
        )
        self.problem_text.grid(row=1, column=0)

        self.problem__answer_text_label = Label(
            frame_top, text="Ответ: ", font=("Arial", 14)
        )
        self.problem__answer_text_label.grid(
            row=2, column=0, columnspan=2, padx=10, sticky="w"
        )

        self.problem__answer_text = Text(
            frame_top, width=80, height=1, font=font_size_text
        )
        self.problem__answer_text.grid(row=3, column=0)

        self.program_problem_text_label = Label(
            frame_top, text="Решение ученика:", font=font_size_label
        )
        self.program_problem_text_label.grid(
            row=4, column=0, columnspan=2, padx=10, sticky="w"
        )

        self.program_text = HTMLScrolledText(
            frame_top, width=80, height=15, font=font_size_text
        )
        ...
        self.program_text.bind(
            "<KeyRelease>", lambda event: self.change(self.program_text)
        )
        # print(1)
        ...
        self.program_text_stdout_label = Label(
            frame_top, text="Вывод программы:", font=font_size_label
        )
        self.program_text_stdout_label.grid(
            row=7, column=0, columnspan=2, padx=10, sticky="w"
        )

        self.program_stdout = Text(
            frame_top, width=80, height=3, font=font_size_text
        )
        self.program_stdout.grid(row=8, column=0, padx=5, pady=5, sticky="we")

        self.program_text_stderr = Label(
            frame_top, text="Ошибки:", font=font_size_label
        )
        self.program_text_stderr.grid(
            row=9, column=0, columnspan=2, padx=10, sticky="w"
        )

        self.program_stderr = Text(
            frame_top, width=80, height=3, font=font_size_text
        )
        self.program_stderr.grid(row=10, column=0, padx=5, pady=5, sticky="we")

        self.progeram_text_return_code = Label(
            frame_top, text="Код возврата:", font=font_size_label
        )
        self.progeram_text_return_code.grid(
            row=11, column=0, columnspan=2, padx=10, sticky="w"
        )

        self.program_return_code = Text(
            frame_top, width=80, height=1, font=font_size_text
        )
        self.program_return_code.grid(
            row=12, column=0, padx=5, pady=5, sticky="we"
        )

        self.program_text_teacher_comment = Label(
            frame_top, text="Комментарий учителя:", font=font_size_label
        )
        self.program_text_teacher_comment.grid(
            row=13, column=0, columnspan=2, padx=10, sticky="w"
        )

        self.program_teacher_comment_entry = Entry(
            frame_top, width=80, font=font_size_text
        )
        self.program_teacher_comment_entry.grid(
            row=14, column=0, padx=5, pady=5, sticky="we"
        )
    ...
    def safe_teacher_comment(self):
        ...
        list(self.employee_list.items())[self.row][1][
            self.employee_list["task"][self.column - 1]
        ]["teacher_comment"] = self.program_teacher_comment_entry.get()
    ...
    def highlight_python_code(self, python_code):
        ...
        highlighted_code = highlight(
            python_code,
            lexer,
            pf.HtmlFormatter(style="lightbulb", noclasses=True),
        )
    ...
    def update_code_student(self, row, column):
        ...
        if self.row != None and self.column != None:
            if (
                self.column > 0
                and self.column < len(self.employee_list["task"]) + 1
            ):
        ...
        if self.column < 1 and self.column > len(self.employee_list["task"]):
            return
        self.problem_text_label.config(
            text="Задача :  " + self.employee_list["task"][column - 1]
        )

        code = list(self.employee_list.items())[row][1][
            self.employee_list["task"][column - 1]
        ]
        highlighted_code = self.highlight_python_code(
            code["student_code"] if code["student_code"] != None else ""
        )
        self.program_text.set_html(highlighted_code, strip=False)
        ...
        self.program_problem_text_label.config(
            text="Решение  {:{l}} : {:8}".format(
                self.student_name,
                code["task_path_name"],
                l=self.max_len_student_name,
            )
        )

        self.program_text_stdout_label.config(
            text="Вывод программы:" + " " * 30 + "Время :" + execution_time
        )
        ...
        stdout = list(self.employee_list.items())[row][1][
            self.employee_list["task"][column - 1]
        ]["student_stdout"]
        self.program_stdout.delete(1.0, END)
        self.program_stdout.insert(END, stdout if stdout != None else "")

        stderr = list(self.employee_list.items())[row][1][
            self.employee_list["task"][column - 1]
        ]["student_stderr"]
        self.program_stderr.delete(1.0, END)
        self.program_stderr.insert(END, stderr if stderr != None else "")

        return_code = list(self.employee_list.items())[row][1][
            self.employee_list["task"][column - 1]
        ]["student_return_code"]
        self.program_return_code.delete(1.0, END)
        self.program_return_code.insert(
            END, return_code if return_code != None else ""
        )

        teacher_comment = list(self.employee_list.items())[row][1][
            self.employee_list["task"][column - 1]
        ]["teacher_comment"]
        self.program_teacher_comment_entry.delete(0, END)
        self.program_teacher_comment_entry.insert(
            0, teacher_comment if teacher_comment != None else ""
        )
    ...
    def set_teacher_comment(self):
        if self.row != None:
            print(1)
            employee_list = self.t.get_employee()
            list(employee_list.items())[self.row][1][
                self.employee_list["task"][self.column - 1]
            ]["teacher_comment"] = self.program_teacher_comment_entry.get()
            self.t.set_employee(employee_list)
            print(self.program_teacher_comment_entry.get())
