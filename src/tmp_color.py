...
class Application(Tk):
    ...
    def open_directory_dialog(self):
        ...
        if self.filename:
            ...
            self.entry_student_folder.insert(
                0, self.filename
            )  # Вставляем выбранный путь в поле ввода
    ...
    def on_class_number_combobox_change(self, event):
        elements = os.listdir(
            self.folder_path + "\\" + self.class_number_combobox.get()
        )
        for i in elements:
            if i != "task and data" and i != "tmp":
                elements = os.listdir(
                    self.folder_path
                    + "\\"
                    + self.class_number_combobox.get()
                    + "\\"
                    + i
                )
                self.tmp_student = i
    ...
    def on_month_number_combobox_change(self, event):
        elements = os.listdir(
            ...
            + self.tmp_student
            + "\\"
            + self.month_number_combobox.get()
        )
    ...
    def submit(self):
        ...
        try:
            setting_data["current"] = [
                setting_data["current"][0],
                setting_data["month"],
                setting_data["date"],
                setting_data["task_list"][self.class_name][self.date][0],
            ]
        except:
            setting_data["current"] = [
                setting_data["current"][0],
    ...
    def update_TL(self):
        try:
            t = float(self.execution_time_limit.get(1.0, END))
            self.t.update_time_limit(t)
            print(t)
        except Exception as e:
            print("Error get TL", e)
    ...
    def create_widgets(self):
        ...
        font_size = ("Arial", 12)
        frame_data = Frame()
        class_number_label = Label(
            frame_data, text="Базовая папка:", font=font_size
        )
        ...
        self.entry_student_folder.grid(
            row=0, column=1, columnspan=5, padx=0, pady=0, sticky=W + E
        )
        ...
        class_number_label = Label(
            frame_data, text="Номер класса:", font=font_size
        )
        class_number_label.grid(row=1, column=0, padx=5, pady=5)
        self.class_number_combobox = ttk.Combobox(
            frame_data, font=font_size, width=7
        )
        self.class_number_combobox.set(self.class_name)
        self.class_number_combobox.grid(row=1, column=1, padx=5, pady=5)
        self.class_number_combobox.bind(
            "<<ComboboxSelected>>", self.on_class_number_combobox_change
        )

        month_number_label = ttk.Label(
            frame_data, text="Номер месяца:", font=font_size
        )
        month_number_label.grid(row=1, column=2, padx=5, pady=5)
        self.month_number_combobox = ttk.Combobox(
            frame_data, font=font_size, width=10
        )
        self.month_number_combobox.set(self.month)
        self.month_number_combobox.grid(row=1, column=3, padx=5, pady=5)
        self.month_number_combobox.bind(
            "<<ComboboxSelected>>", self.on_month_number_combobox_change
        )
        ...
        self.date_combobox.bind(
            "<<ComboboxSelected>>", self.on_date_combobox_change
        )

        self.submit_button = Button(
            frame_data, text="Загрузить", command=self.submit, font=font_size
        )
        self.submit_button.grid(row=1, column=6, padx=5, pady=5)

        self.student_task_label = Label(
            frame_data, text="Задачи:", font=font_size
        )
        self.student_task_label.grid(row=2, column=0)

        self.student_task_text = Text(
            frame_data, height=1, width=60, font=font_size
        )
        print(self.employee_list["task"])
        self.student_task_text.insert(END, self.employee_list["task"])
        self.student_task_text.grid(row=2, column=1, columnspan=5, sticky="w")

        self.student_task_entry = Button(
            frame_data, text="Загрузить", command=self.load_task, font=font_size
        )
        self.student_task_entry.grid(row=2, column=6, padx=5, pady=5)

        self.student_name_label_progress = Label(
            frame_data, text="Ученик :", font=font_size
        )
        self.student_name_label_progress.grid(
            row=3, column=0, columnspan=2, padx=5, pady=5, sticky="w"
        )

        self.task_name_label_progress = Label(
            frame_data, text="Задача :", font=font_size
        )
        ...
        self.execution_time_limit = Text(
            frame_data, height=1, width=8, font=font_size
        )
        self.execution_time_limit.insert(END, 2)
        self.execution_time_limit.grid(row=3, column=5, padx=5, pady=5)

        self.submit_button = Button(
            frame_data, text="Set TL", font=font_size, command=self.update_TL
        )
        self.submit_button.grid(row=3, column=6, padx=5, pady=5)

        self.run_code = Button(
            frame_data, text="Run", font=font_size, command=self.run_code
        )
        self.run_code.grid(row=4, column=6, padx=5, pady=5)

        self.safe_response = Button(
            frame_data,
            text="SR",
            font=font_size,
            command=self.safe_srudent_response,
        )
        self.safe_response.grid(row=4, column=5, padx=5, pady=5)

        self.safe_open_response = Button(
            frame_data, text="OR", font=font_size, command=self.t.open_result
        )
        self.safe_open_response.grid(row=4, column=4, padx=5, pady=5)

        self.open_date_response_text = Text(
            frame_data, height=1, width=60, font=font_size
        )
        self.open_date_response_text.grid(
        ...
        self.safe_open_response = Button(
            ...
            text="OR all run",
            font=font_size,
            command=self.open_result_all_run,
        )
        self.safe_open_response.grid(row=5, column=3, padx=5, pady=5)

        self.safe_open_response = Button(
            frame_data, text="CW", font=font_size, command=self.t.open_c_work
        )
        ...
        # Создаем таблицу
        self.table = Table(
            table_frame, self.employee_list, self.frame, self, self.t
        )
