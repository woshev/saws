...
class Task:
    def __init__(self, app=None):
        ...
        self.folder_path = (
            setting_data["folder_path"] + "\\" + setting_data["class"]
        )
        ...
        self.time_limit = setting_data["time_limit"]
    ...
    def folder_path_student_get(self, row, column):
        print(list(self.employee_list.items())[row][0])
        ...
        path = (
            self.folder_path
            + "\\"
            + list(self.employee_list.items())[row][0]
            + "\\"
            + self.month
            + "\\"
            + self.date
    ...
    def update_time_limit(self, time):
        ...
        with open(os.getcwd() + "\\setting.yml", "r", encoding="utf-8") as file:
            setting_data = yaml.safe_load(file)
        setting_data["time_limit"] = self.time_limit
    ...
    def build_path_tree(self, folder_path):
        ...
        with os.scandir(folder_path) as entries:
            for entry in entries:
                if (
                    entry.is_dir()
                    and entry.name not in self.list_stop_path_name
                ):
            ...
            if paths[current[0]][current[1]][current[2]] != {}:
                ...
                for i in paths[current[0]][current[1]][current[2]]["files"]:
                    ...
                    if (
                        task != None
                        and topic != None
                        and str(topic) + "-" + str(task) == current[-1]
                    ):
                        task_name = i
                        break
                    elif topic == None and current[-1].split("-")[1] == str(
                        task
                    ):
                        task_name = i
                        break
    ...
    def next_file(self, paths: dict, current: list, task_list: list):
        ...
        if task_list.index(current[-1]) + 1 < len(task_list):
            ...
            if student_list.index(current[0]) + 1 < len(student_list):
                ...
                if paths[current[0]][current[1]][current[2]] != {} and paths[
                    current[0]
                ][current[1]][current[2]]["files"] != ["result.txt"]:
                    task_name = paths[current[0]][current[1]][current[2]][
                        "files"
                    ][task_list.index(current[-1])]
                else:
                    task_name = ""
    ...
    def next_file_c_work(self, paths: dict, current: list, task_list: list):
        ...
        if task_list.index(current[-1]) + 1 < len(task_list):
            ...
            if student_list.index(current[0]) + 1 < len(student_list):
                ...
                if current[-1] == task_list[-1]:
                    return False, current, ""
                if paths[current[0]] != {} and paths[current[0]]["files"] != [
                    "result.txt"
                ]:
                    task_name = paths[current[0]]["files"][
                        task_list.index(current[-1])
                    ]
                else:
                    task_name = ""
        ...
        try:
            ...
            if task_description["input file"] != None:
                path_input_file = os.path.join(
                    str(task_description["ege type"]) + "data",
                    task_description["input file"],
                )
                    ...
                    for task_name, task in student_value.items():
                        if task["task_path_name"] != None:
                            ...
                            (
                                ...
                                correct_ans,
                                code,
                            ) = self.run_student_code(current, task_list)
                            print(
                                "Ученик {:<15}  Задача  {:<20}  Прогресс {:7.2f}".format(
                                    student_name,
                                    task_name,
                                    count / count_all * 100,
                                )
                    ...
                    for task_name, task in student_value.items():
                        if task["task_path_name"] != None:
                            current = [
                                ...
                                self.date,
                                task_name,
                                self.employee_list[student_name][task_name][
                                    "task_path_name"
                                ],
                            ]
                            (
                    ...
                    for task_name, task in student_value.items():
                        if task["task_path_name"] != None:
                            ...
                            (
                                ...
                                correct_ans,
                                code,
                            ) = self.run_student_code_c_work(current, task_list)
                            print(
                                "Ученик {:<15}  Задача  {:<20}  Прогресс {:7.2f}".format(
                                    student_name,
                                    task_name,
    ...
    def run_student_code(self, current: list, task_list: list):
        ...
        if self.copy_code_and_data(path_code_student, task_description):
            with open(
                self.tmp_folder + "\\" + current[-1], "r", encoding="utf-8"
            ) as tmp:
                code = tmp.read()
            stdout, stderr, return_code, execution_time = self.run_code(
                self.tmp_folder + "\\" + current[-1], self.time_limit
            )

            if type(self.data[current[3]]["answer"]) == list:
                print(repr(stdout), "\n", repr(self.data[current[3]]["answer"]))
                print(stdout.split("\n")[:-1])
            else:
                print(repr(stdout))
                print(repr(self.data[current[3]]["answer"]))
                print(repr(stdout.split("\n")[0]))
            ...
            if execution_time == False:
                ...
                if type(self.data[current[3]]["answer"]) == list:
                    ...
                    if self.data[current[3]]["answer"] == stdout.split("\n")[0]:
                        return (
                            True,
                            stdout,
                            stderr,
    ...
    def run_student_code_c_work(self, current: list, task_list: list):
        ...
        if self.data == None:
            self.data = self.open_yaml(task_list)
        path_code_student = (
            self.folder_path + "\\" + current[0] + "\\" + current[-1]
        )
    ...
    def run_code(self, path_code, timeout=2):
        ...
        process = subprocess.Popen(
            ["python", os.path.basename(path_code)],
            cwd=self.tmp_folder,
            text=True,
                            ...
                            if tk.messagebox.askyesno(
                                ...
                                with open(
                                    ...
                                    for (
                                        task_name,
                                        task,
                                    ) in student_value.items():
                                        l.append(
                                            len(
                                                task["task_path_name"]
                                                if task["task_path_name"]
                                                != None
                                                else " "
                                            )
                                        )
                                    ...
                                    print(
                                        "teacher com", task["teacher_comment"]
                                    )
                                    for (
                                        task_name,
                                        task,
    ...
    def get_employee(self):
        print(
            self.employee_list["task"] if self.employee_list != None else "None"
        )
    ...
    def create_list_task(self):
        ...
        tmp = current[0]
        tmp_list = {}
        b, current, task = self.current_file(
            self.path_tree, current, self.task_list
        )
        while b:
    ...
    def create_list_task_all(self, list_date):
        ...
        for cur_date in list_date:
            if self.class_name in setting_data["task_list"]:
                if (
                    self.date
                    in setting_data["task_list"][self.class_name].keys()
                ):
                    l = 0
                    for i in setting_data["task_list"][self.class_name][
                        cur_date
                    ]:
                    ...
                    if current[0] == self.student_list[-1]:
                        if b:
                            try:
                                if (
                                    current[-1]
                                    == self.task_list[t + self.amount[cur_date]]
                                ):
                                    b = False
                                    current[-1] = self.task_list[
                                        self.amount[cur_date]
                                    ]
                        ...
                        for task_name, task in student_value.items():
                            ...
                            if count > sl:
                                ...
                                if len(tmp_line[0].split()) > 1:
                                    task["task_path_name"] = " ".join(
                                        tmp_line[0].split()[1:]
                                    )
                                    t = [
                                        tmp_line[0].split()[0],
                                        " ".join(tmp_line[0].split()[1:]),
                                    ]
                                else:
                                    if tmp_line[1] != "":
                                        if task["task_path_name"] == None:
                                            task["task_path_name"] = tmp_line[
                                                1
                                            ].split()[0]
                                            t = [
                                                tmp_line[0].split()[0],
    ...
    def create_list_task_control_work(self):
        ...
        tmp = current[0]
        tmp_list = {}
        b, current, task = self.current_file_c_work(
            self.path_tree, current, self.task_list
        )
        while b:
            # if current[0] == 'Дикаева Дарья':
            ...
            if task == None:
                ...
                with open(
                    self.folder_path + "\\" + current[0] + "\\" + task,
                    "r",
                    encoding="utf-8",
                ) as file:
                    code = file.read()
            ...
            b, current, task = self.next_file_c_work(
                self.path_tree, current, self.task_list
            )
            if current[0] != tmp or not b:
                student_task[tmp] = tmp_list
